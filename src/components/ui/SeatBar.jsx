...
// Stacked seat bar, election-broadcast style. Blue fills from the left, red
// from the right, green (3-party) after blue. `tossup`: districts whose
// undecided (grey) population could still flip them — their own hatched
// segment between the camps. `target`: the goal seat count — the tick marks
// where you need to reach (not a fixed majority line).
export default function SeatBar({ seats, numDistricts, isThreeParty = false, tossup = 0, target = null }) {
  ...
  const tickLeft = target != null && numDistricts > 0
    ? `${Math.min(target / numDistricts, 1) * 100}%`
    : '50%';
  ...
  return (
    <div className="seat-bar" role="img" aria-label={label}>
      ...
      <div
        className="seat-bar__tick"
        style={{ left: tickLeft }}
        title={target != null ? `Target: ${target}/${numDistricts} seats` : 'Majority'}
      />
    </div>
  );
}
